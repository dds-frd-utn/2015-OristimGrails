	...
	<body>
		...
		<div id="show-carrito" class="content scaffold-show" role="main">
			...
			<ol class="property-list carrito">
				...
				<g:if test="${carritoInstance?.juegos}">
				<li class="fieldcontain">
					<span id="juegos-label" class="property-label"><g:message code="carrito.juegos.label" default="Juegos" /></span>
					
						<g:each in="${carritoInstance.juegos}" var="j">
						<span class="property-value" aria-labelledby="juegos-label"><g:link controller="juego" action="show" id="${j.id}">${j?.encodeAsHTML()}</g:link></span>
						</g:each>
